...
public class Main { //Creating class
	...
	public static void customerPurchaseOfStock(String[] args) {
		// Variables //
		...
		if (checkAccountID(providedID)) {
			...
			if (checkDuplicateTicker(stocks, providedTicker)) { // Checking to see if the stock exists
				while (!inputAssured) {
					try {
						System.out.print("How many stocks do you wish to buy: ");
						sharesBought = userInput.nextInt();


						if (sharesBought < 0) {
							System.out.println(5/0); // Divide by 0 to trigger catch
						} // Close if
				...
				return; // If the stock doesnt exist reutrn
			}
		} else {
